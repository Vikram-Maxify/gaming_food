    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* 🔥 RECENT ORDERS */}
          <div className="bg-cardGradient border border-borderSubtle rounded-xl2 p-5">
            <h3 className="text-textPrimary mb-4">Recent Orders</h3>

            {recentOrders.map(order => {
              const total = order.items.reduce((s,i)=>s+i.price*i.quantity,0);

              return (
                <div key={order._id} className="flex justify-between items-center p-3 mb-2 bg-[#1A1A1A] rounded-lg">

                  <div className="flex gap-3 items-center">
                    <img src={order.items[0]?.product?.image} className="w-10 h-10 rounded-lg object-cover" />
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs text-textSecondary">{order.status}</span>
                  </div>
